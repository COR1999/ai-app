...
export const projects: Project[] = [
  {
    ...
    shortDescription: "Open-source dashboard for monitoring free AI model availability across NVIDIA NIM and OpenCode — because they update the list without telling anyone.",
    fullDescription: "An open-source Next.js dashboard that lets anyone check which free-tier AI models are currently live on NVIDIA's NIM API and OpenCode. NVIDIA regularly adds, removes, and sunsets models from their free catalog without announcements, changelogs, or any form of notification — so developers are left guessing which endpoints actually work. This tool hits every model with a real test request and shows you what's up, what's down, and what's new. Features include model comparison, a 30-day changelog of additions/removals, dark/light theme, mobile-responsive card layout, 7-day uptime tracking, function-calling detection, T3 Code compatibility badges, and shareable snapshot links. Built out of frustration with stale docs and broken endpoints.",
    technologies: ["Next.js", "React", "TypeScript", "Tailwind CSS", "NVIDIA NIM API", "OpenCode API", "Vercel"],
    features: [
      "Live availability testing for every free NVIDIA NIM and OpenCode model via real API calls",
      "Model comparison — select up to 3 models and compare status, response time, tools, and uptime side-by-side",
      "30-day changelog tracking model additions and removals over time (localStorage)",
      "Dark/light theme toggle with full color palette support",
      "Mobile-responsive card layout for testing on the go",
      "Category filters — chat, code, vision, embedding, audio",
      "7-day uptime history tracked per model for reliability insights",
      "Function-calling detection via tools payload in test requests",
      "T3 Code compatibility badges for models known to break with Chat Completions",
      "NVIDIA model links to build.nvidia.com for every model",
      "New model alerts — badges models that appeared since your last visit",
      "Shareable snapshot links to share current status with others",
      "Daily Vercel Cron job for automated testing at 8am UTC",
      "Batched test-all to stay under Vercel's serverless function timeout limits",
      "Known-slow models skipped automatically during test-all to save time"
    ],
